...
export const StepEditor: React.FC = () => {
  const { config, activePageId, updatePage, updateEvents, deletePage, reorderPages, downloadPageConfig, copyToClipboard } = useFunnel();
            ...
            <div className="space-y-8">
                ...
                <section>
                    ...
                    {(() => {
                        ...
                        return (
                            <div className="space-y-3">
                                {/* Current position with manual override */}
                                <div className="flex items-center gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            if (isNaN(newPos) || newPos < 1 || newPos > allPages.length) return;
                                            const currentPos = parseInt(activePage.path);
                                            if (newPos === currentPos) return;
                                            // Build new order by moving this page to the new position
                                            const pageKeys = Object.keys(config.pages).filter(k => k !== '__template_preview__');
                                            const fromIdx = pageKeys.indexOf(activePageId);
                                            if (fromIdx === -1) return;
                                            const newOrder = [...pageKeys];
                                            newOrder.splice(fromIdx, 1);
                                            newOrder.splice(newPos - 1, 0, activePageId);
                                            reorderPages(newOrder);
                                        }}
                                        className="font-mono text-sm font-bold text-gray-800 bg-white px-2 py-0.5 rounded border border-gray-200 w-14 text-center focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none"
                                    />
