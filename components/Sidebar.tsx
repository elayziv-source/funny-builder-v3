...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange }) => {
  ...
  const dragOriginalOrderRef = useRef<string[] | null>(null);
  ...
  const handleDragStart = (e: React.DragEvent, id: string) => {
      ...
      dragOriginalOrderRef.current = keys;
      setVisualOrder(keys);
      setDraggedItem(id);
  ...
  const handleDragOver = (e: React.DragEvent, targetId: string) => {
      e.preventDefault();
      if (!draggedItem || draggedItem === targetId || !visualOrder) return;

      const fromIndex = visualOrder.indexOf(draggedItem);
      const toIndex = visualOrder.indexOf(targetId);
  ...
  const handleDrop = (e: React.DragEvent) => {
      e.preventDefault();
      if (visualOrder && dragOriginalOrderRef.current) {
          // Only apply reorder if order actually changed
          const originalOrder = dragOriginalOrderRef.current;
          const changed = visualOrder.some((key, i) => key !== originalOrder[i]);
          if (changed) {
              reorderPages(visualOrder);
          }
      }
      setDraggedItem(null);
      setVisualOrder(null);
  ...
  // During drag, use visualOrder for display; otherwise use config order
  const displayOrder = useMemo(() => {
      const order = visualOrder || Object.keys(config.pages);
      return order.filter(key => key !== '__template_preview__' && config.pages[key]);
  }, [visualOrder, config.pages]);

  const filteredPages = displayOrder
    .map(key => [key, config.pages[key]] as [string, PageConfig])
    .filter(([key, page]) =>
      (page?.name || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
      key.toLowerCase().includes(searchTerm.toLowerCase())
    );

  // During drag, compute preview paths based on visual order
  const dragPreviewPaths = useMemo(() => {
      if (!visualOrder) return null;
      const paths: Record<string, string> = {};
      visualOrder.filter(k => k !== '__template_preview__').forEach((key, idx) => {
          paths[key] = (idx + 1).toString();
      });
      return paths;
  }, [visualOrder]);
    ...
    <div className="flex flex-col h-full bg-[#1a1c23] text-white w-64 border-r border-gray-800 flex-shrink-0 z-30">
      ...
      <div className="flex-1 overflow-y-auto px-2 space-y-0.5 custom-scrollbar min-h-0">
        {filteredPages.map(([key, page]) => {
          const displayPath = dragPreviewPaths?.[key] || page?.path || '?';
          const pathChanged = dragPreviewPaths && dragPreviewPaths[key] !== page?.path;
          return (
          <div
            key={key}
          ...
          >
            ...
            <span className={`font-mono text-[9px] w-4 h-4 flex items-center justify-center rounded flex-shrink-0 ${pathChanged ? 'bg-amber-600 text-white' : activePageId === key ? 'bg-gray-700 text-gray-300' : 'bg-gray-800 text-gray-600'}`}>{displayPath}</span>
            <span
              className="w-1.5 h-1.5 rounded-full flex-shrink-0"
              style={{ backgroundColor: getQuizPhase(displayPath).color }}
              title={`${getQuizPhase(displayPath).name} Phase`}
            />
