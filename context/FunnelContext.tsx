...
export const FunnelProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const reorderPages = (newOrder: string[]) => {
      // Compute the reorder result synchronously for toast, then apply via setConfig
      const currentConfig = configRef.current;
      const newPagesRaw: Record<string, PageConfig> = {};
      newOrder.forEach(key => {
          if (currentConfig.pages[key]) {
              newPagesRaw[key] = currentConfig.pages[key];
          }
      });
      const reindexed = reindexPages(newPagesRaw);
      const { routing: updatedRouting, sequentialCount, customCount } = reindexEventRouting(
          currentConfig.pages, reindexed, currentConfig.event_routing
      );
      ...
      const totalRoutes = Object.values(currentConfig.event_routing || {}).filter(
          (e: any) => e?.route?.to
      ).length;
      if (sequentialCount > 0 || customCount > 0) {
          const parts: string[] = [];
          if (sequentialCount > 0) parts.push(`${sequentialCount} sequential route${sequentialCount > 1 ? 's' : ''} updated`);
          if (customCount > 0) parts.push(`${customCount} custom route${customCount > 1 ? 's' : ''} preserved`);
          notify(`Routing updated: ${parts.join(', ')}`, 'success');
      } else if (totalRoutes > 0) {
          notify('Pages reordered. No routing changes needed.', 'info');
      } else {
          notify('Pages reordered.', 'info');
      }
  };
